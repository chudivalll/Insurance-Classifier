...
categorical1 = pd.DataFrame(pandasql.sqldf('''SELECT x35,
                                          CASE
...
categorical1 = categorical1.drop('x35', axis = 1, inplace = False).rename(columns = {'xx35':'x35'}, inplace = False)
...
train_df_features, test_df_features, train_df_labels, test_df_labels = train_test_split(df_features, df_labels, test_size = 0.25, random_state = 42)
...
print(k_scores)  #Of the 30 values tested for k, k=7 produced the best results with an accuracy score of 0.937
#%%
